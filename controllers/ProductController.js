...
class ProductController {
    static index = async (req, res) => {
        try {
            ...
            const item_per_page = process.env.PRODUCT_ITEM_PER_PAGE;//số lượng sản phẩm mỗi trang
            let conds = [];//không có điều kiện
            let sorts = [];

            // Tìm kiếm theo danh  mục
            // /danh-muc/kem-tri-mun/c3.html
            ...
            if (priceRange) {
                ...
                // SELECT * FROM view_product WHERE category_id=3 AND sale_price BETWEEN 500000 AND 1000000
                if (end == 'greater') {
                    conds = {
                        ...conds,
                        'sale_price': {
            ...
            if (sort) {
                ...
                // sorts[colName] = order;
                // cần chuyển giá trị của biến thành thuộc tính thì dùng dấu ngoặc vuông
                sorts = {
                    // sale_price: ASC
                    [colName]: order
                }
